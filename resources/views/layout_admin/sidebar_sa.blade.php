    <!-- Begin page -->
    <div class="wrapper">
        <!-- ========== Left Sidebar Start ========== -->
        <div class="leftside-menu">

            <!-- LOGO -->
            <a href="/superadmin" class="logo text-center logo-light">
                <span class="logo-lg">
                    <img src="assets/img/desacantik.png" width="50" height="50"> Admin Desa Cantik
                </span>
            </a>

            <!-- LOGO -->
            <a href="/superadmin" class="logo text-center logo-dark">
                <span class="logo-lg">
                    <img src="assets/img/desacantik.png" width="50" height="50"> Admin Desa Cantik
                </span>
            </a>

            <div class="h-100" id="leftside-menu-container" data-simplebar="">

                <!--- Sidemenu -->
                <ul class="side-nav">

                    <li class="side-nav-title side-nav-item">Navigation</li>

                    <li class="side-nav-item">
                    <li class="side-nav-item">
                        <a href="/superadmin" class="side-nav-link">
                            <i class="uil-home-alt"></i>
                            <span> Dashboard</span>
                        </a>
                    </li>

                    <li class="side-nav-title side-nav-item">Kelola Rumah Tangga</li>
                    <li class="side-nav-item">
                        <a href="/kartukeluarga" class="side-nav-link">
                            <i class="dripicons-home"></i>
                            <span> Kartu Keluarga </span>
                        </a>
                    </li>
                    <li class="side-nav-item">
                        <a href="/anggotaruta" class="side-nav-link">
                            <i class="dripicons-checklist"></i>
                            <span> Anggota Rumah Tangga </span>
                        </a>
                    </li>

                    <li class="side-nav-title side-nav-item">Kelola Website</li>
                    <li class="side-nav-item">
                        <a href="/createfoto" class="side-nav-link">
                            <i class="dripicons-camera"></i>
                            <span> Tambah Foto</span>
                        </a>
                    </li>
                    <li class="side-nav-item">
                        <a href="/berita" class="side-nav-link">
                            <i class="dripicons-blog"></i>
                            <span> Tambah Berita</span>
                        </a>
                    </li>
                    <li class="side-nav-item">
                        <a href="/profil-desa" class="side-nav-link">
                            <i class="dripicons-blog"></i>
                            <span> Monografi </span>
                        </a>
                    </li>

                    <li class="side-nav-item">
                        <a href="/komentars" class="side-nav-link">
                            <i class="dripicons-blog"></i>
                            <span> Manajemen Komentar </span>
                        </a>
                    </li>

                    <li class="side-nav-title side-nav-item"> Kelola pengguna</li>
                    <li class="side-nav-item">
                        <a href="/kelolapengguna" class="side-nav-link">
                            <i class="dripicons-user"></i>
                            <span> Kelola Pengguna</span>
                        </a>
                    </li>
                    <li class="side-nav-item">
                        <a href="/register" class="side-nav-link">
                            <i class="dripicons-user"></i>
                            <span> Registrasi </span>
                        </a>
                    </li>
                </ul>
                <!-- End Sidebar -->

                <div class="clearfix"></div>

            </div>
            <!-- Sidebar -left -->

        </div>
        <!-- Left Sidebar End -->